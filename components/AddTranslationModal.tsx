...
export default function AddTranslationModal({ selectedText, projectId, onClose, onSaved }: AddTranslationModalProps) {
  ...
  const handleSave = async () => {
    ...
    await fetch(`/api/projects/${projectId}/translations`, {
      ...
      body: JSON.stringify({ original_text: selectedText, translation, question }),
    });
    setSaving(false);
    onSaved();
    onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-slate-900 rounded-lg border border-slate-700 shadow-2xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between p-5 border-b border-slate-700">
          <h2 className="text-white font-semibold text-lg">Añadir Traducción</h2>
